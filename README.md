...
0.1.7 补充工作流所属进程组诊断：区分已退出的启动命令和仍存活的后代进程，避免把根命令数为 0 误解为工作流空闲。明确结束旧工作流时可通过 `resume → status → end` 回收其资源，无需重启其他项目；空闲自动回收的保护条件保持不变。

0.1.8 修复搜索清理跨项目阻塞、浏览器和 WebView2 启停竞争、过期预约激活及配置保存竞争。工作流工具的每个动作声明完整参数，兼容按分支生成调用签名的客户端。更新运行包后应刷新插件工具目录；已有任务可能仍缓存旧签名。
...
每次新的 `write_stdin` 轮询都应使用新的 `request_id`，即使输入为空；只有重试同一次丢失响应的轮询时复用原 ID，否则会读到该次轮询保存的旧结果。
